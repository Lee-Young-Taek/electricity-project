# ============================ viz/appendix_eda.py (FINAL, de-duplicated) ============================
...
_PALETTE = {
    "bg": "#f6f8fb", "panel": "#ffffff", "font": "#1e293b", "muted": "#64748b",
    "grid": "#e2e8f0", "accent": "#2563eb", "line": "#123b78", "avg": "#3b82f6",
    "warn": "#f59e0b", "danger": "#ef4444", "weekday":"#94a3b8",
    "cost_cur": "#2563eb", "cost_prev": _rgba("#2563eb", 0.35),
    "kwh_cur": "#10b981", "kwh_prev": _rgba("#10b981", 0.35),
}
_PALETTE.setdefault("primary", _PALETTE["accent"])

def _apply_layout(fig: go.Figure, title: str = "", height: int = 420) -> go.Figure:
    fig.update_layout(
        title=title, height=height, paper_bgcolor="white", plot_bgcolor="white",
        font=dict(family="Noto Sans KR, Inter, Arial, system-ui, sans-serif", size=12, color=_PALETTE["font"]),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0),
        hovermode="x unified", margin=dict(l=48, r=24, t=60, b=48),
    )
    fig.update_xaxes(showgrid=True, gridcolor=_PALETTE["grid"], zeroline=False, ticks="outside")
    fig.update_yaxes(showgrid=True, gridcolor=_PALETTE["grid"], zeroline=False, ticks="outside")
    return fig

# -------- utils --------
def _to_dt(x) -> pd.Series:
    s = pd.Series(x) if not isinstance(x, (pd.Series, pd.DatetimeIndex, pd.Index)) else pd.Series(x)
...
def _safe_replace_year(dt_like, year: int) -> pd.Series:
    ...
    out = []
    for ts in s:
        if pd.isna(ts): out.append(pd.NaT); continue
...
def _holidays_by_year(year: int) -> set:
    ...
    if year == 2018:
        ...
        d.update(map(lambda x: pd.Timestamp(x).date(), [
            ...
            "2022-06-01","2022-06-06","2022-10-03","2022-10-09","2022-12-25"
        ]))
...
def render_calendar_alignment_storyline(df: pd.DataFrame):
    ...
    d = df[["측정일시"]].copy()
    d["측정일시"] = _to_dt(d["측정일시"]).dropna().sort_values()
...
def render_calendar_overlay(df: pd.DataFrame, year: int = 2018,
                            highlight_weekend: bool = True, highlight_holiday: bool = True):
    ...
    fig = go.Figure()
    fig.add_scatter(
        x=pd.to_datetime(daily["date"]), y=daily["전기요금(원)"], mode="lines",
        name="일별 합계", line=dict(width=2, color=pal["line"]),
        hovertemplate="<b>%{x|%Y-%m-%d}</b><br>전기요금: %{y:,.0f}원<extra></extra>",
    )
    for key, color in [("공휴일", pal["danger"]), ("주말", pal["warn"]), ("평일", pal["weekday"])]:
        ...
        fig.add_scatter(
            ...
            marker=dict(color=color, size=10, line=dict(color=pal["panel"], width=1.2), opacity=0.95),
            hovertemplate="<b>%{x|%Y-%m-%d}</b><br>%{meta}: %{y:,.0f}원<extra></extra>", meta=key,
        )
...
def render_midnight_rollover_fix(df: pd.DataFrame):
    ...
    n_total, n_midnight = len(d), len(midnight_data)
    ...
    <div class="billx-panel">
      ...
      <div class="alert alert-warning mb-3"><strong>검출:</strong> 00:00 시각 데이터 <strong>{n_midnight:,}건</strong> 발견 (전체의 {pct}%)</div>
      <ul class="mb-3">
        <li>00:00은 다음날 00:00으로 기록된 것으로 추정 → 날짜 경계 보정 필요(+1일)</li>
      </ul>
...
def render_basic_stats(df: pd.DataFrame):
    ...
    return ui.HTML(
        f"""
        <div class="alert alert-secondary py-2 px-3 mb-2"><strong>요약:</strong> {summary}
          <div class="small text-muted mt-1">※ 기간 표기는 달력 정합성 설명을 위해 2018년 기준으로 변환, 통계는 원시 데이터 기준</div>
        </div>
...
def render_missing_summary(df: pd.DataFrame):
    m = pd.DataFrame({"컬럼": df.columns, "결측수": df.isnull().sum(),
    ...
    return ui.HTML(m.to_html(classes="table table-sm table-striped", index=False, border=0))
...
def render_outlier_summary(df: pd.DataFrame):
    ...
    for c in num_cols:
        ...
        if cnt:
            rows.append({"컬럼": c, "이상치수": cnt, "비율(%)": round(cnt / len(df) * 100, 2),
...
def render_eda_storyline_panels(df: pd.DataFrame):
    ...
    fig_m = go.Figure()
    fig_m.add_bar(x=monthly["month"], y=monthly["전기요금(원)"],
                  text=monthly["전기요금(원)"].apply(lambda x: f"{x:,.0f}"),
                  textposition="outside", marker_color=_PALETTE["cost_cur"])
    _apply_layout(fig_m, title="월별 전기요금 합계 (2018년 기준)", height=360)
    fig_m.update_xaxes(title_text="월"); fig_m.update_yaxes(title_text="전기요금(원)")
    # 일별
    d["date"] = dt2018.dt.date
    daily = d.groupby("date")["전기요금(원)"].sum().reset_index()
    fig_d = go.Figure()
    fig_d.add_scatter(x=daily["date"], y=daily["전기요금(원)"], mode="lines",
                      line=dict(color=_PALETTE["line"], width=2), name="일별 합계")
    _apply_layout(fig_d, title="일별 전기요금 추이 (2018년 기준)", height=360)
    fig_d.update_xaxes(title_text="날짜"); fig_d.update_yaxes(title_text="전기요금(원)")
    # 시간별 평균
    d["hour"] = d["측정일시"].dt.hour
    hourly = d.groupby("hour")["전기요금(원)"].mean().reset_index()
    fig_h = go.Figure()
    fig_h.add_scatter(x=hourly["hour"], y=hourly["전기요금(원)"], mode="lines+markers",
                      line=dict(color=_PALETTE["warn"], width=2), marker=dict(size=8), name="평균")
    _apply_layout(fig_h, title="시간대별 평균 전기요금 (2018년 기준)", height=360)
    fig_h.update_xaxes(title_text="시간"); fig_h.update_yaxes(title_text="평균 전기요금(원)")
    # 계절
    def _season_of_month(m: int) -> str:
        return {12:"겨울",1:"겨울",2:"겨울",3:"봄",4:"봄",5:"봄",6:"여름",7:"여름",8:"여름",9:"가을",10:"가을",11:"가을"}.get(m, "unknown")
    d["season"] = dt2018.dt.month.map(_season_of_month)
    seasonal = d.groupby("season")["전기요금(원)"].mean().reindex(["봄","여름","가을","겨울"]).reset_index()
    fig_s = go.Figure()
    fig_s.add_bar(x=seasonal["season"], y=seasonal["전기요금(원)"],
                  text=seasonal["전기요금(원)"].apply(lambda x: f"{x:,.0f}"),
                  textposition="outside", marker_color=_PALETTE["danger"])
    _apply_layout(fig_s, title="계절별 평균 전기요금 (2018년 기준)", height=360)
    fig_s.update_xaxes(title_text="계절"); fig_s.update_yaxes(title_text="평균 전기요금(원)")
    return ui.div(
        ui.div(
            ui.h5("3. 시계열 패턴 분석", class_="billx-panel-title"),
            ui.div("월/일/시간/계절 관점의 전기요금 패턴으로 시간 기반 피처 설계 근거 확인", class_="alert alert-info mb-3"),
            class_="billx-panel",
        ),
        ui.layout_columns(
...
def plot_distribution(df: pd.DataFrame):
    ...
    if "지상무효전력량(kVarh)" in df:
        fig.add_histogram(x=df["지상무효전력량(kVarh)"], nbinsx=50, showlegend=False, row=2, col=1, marker_color=_PALETTE["weekday"])
    if "지상역률(%)" in df:
...
def plot_correlation_heatmap(df: pd.DataFrame):
    cols = ["전력사용량(kWh)", "지상무효전력량(kVarh)", "진상무효전력량(kVarh)", "지상역률(%)", "진상역률(%)", "탄소배출량(tCO2)", "전기요금(원)"]
    ...
    fig = go.Figure(go.Heatmap(
        ...
        colorscale=[[0.0, _PALETTE["danger"]],[0.5,_PALETTE["panel"]],[1.0,_PALETTE["kwh_cur"]]],
...
def plot_worktype_distribution(df: pd.DataFrame):
    ...
    fig = go.Figure()
    if not vc.empty:
        fig.add_bar(x=vc.index, y=vc.values, text=vc.values, textposition='outside', marker_color=_PALETTE["warn"])
    _apply_layout(fig, title='작업유형별 분포', height=420)
    fig.update_xaxes(title_text='작업유형'); fig.update_yaxes(title_text='건수')
    return ui.HTML(fig.to_html(include_plotlyjs='cdn', full_html=False))

# (추가) 작업유형별 단가 박스플롯
def render_unitprice_box_by_worktype(df: pd.DataFrame):
    need = ["전력사용량(kWh)", "전기요금(원)", "작업유형"]
    if not all(c in df.columns for c in need):
        return ui.div("단가 박스플롯을 위해 ['전력사용량(kWh)', '전기요금(원)', '작업유형'] 필요", class_="p-3 small-muted")
    d = df.copy()
    d["kWh"]  = pd.to_numeric(d["전력사용량(kWh)"], errors="coerce")
    d["bill"] = pd.to_numeric(d["전기요금(원)"], errors="coerce")
    d["unit"] = d["bill"] / (d["kWh"] + 1e-6)
    d = d.replace([np.inf, -np.inf], np.nan).dropna(subset=["unit","작업유형"])
    fig = go.Figure()
    for wt in sorted(d["작업유형"].astype(str).unique()):
        sub = d[d["작업유형"].astype(str) == wt]["unit"]
        if sub.empty: continue
        fig.add_box(y=sub, name=wt, boxmean=True)
    _apply_layout(fig, title="작업유형별 단가 분포(원/kWh)", height=420)
    fig.update_yaxes(title_text="원 / kWh")
    return ui.HTML(fig.to_html(include_plotlyjs='cdn', full_html=False))
...
def render_lag_window_acf(df: pd.DataFrame):
    ...
    fig = go.Figure()
    if not acf_df.empty:
        fig.add_bar(x=acf_df["lag"], y=acf_df["acf"], marker_color=_PALETTE["kwh_cur"],
                    text=acf_df["acf"].apply(lambda x: f"{x:.3f}"), textposition="outside")
    _apply_layout(fig, title="시차 상관관계 분석 (ACF)", height=360)
    fig.update_xaxes(title_text="시차"); fig.update_yaxes(title_text="자기상관계수")
    return ui.HTML(fig.to_html(include_plotlyjs='cdn', full_html=False))
...
def render_holiday_peak_checks(df: pd.DataFrame):
    ...
    for flag in ["오후피크(13-17시)", "저녁피크(18-22시)", "심야(23-05시)"]:
        ...
        stats.append({"시간대": flag, "해당시간 평균(kWh)": round(on_mean, 2),
                      "기타시간 평균(kWh)": round(off_mean, 2), "차이(%)": round(lift, 2)})
...
def render_unitprice_and_logtarget(df: pd.DataFrame):
    ...
    _apply_layout(fig, title="단가 추정 & 로그 타겟 분포", height=420)  # full-width로 사용
    return ui.HTML(fig.to_html(include_plotlyjs='cdn', full_html=False))
